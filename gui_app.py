...
def take_review():
    ...
    rev_tf = Entry(root2, width=100, fg='red', borderwidth=5,
...
def estimate(s):
    ...
    if s == '':
        ...
        conn = pymysql.connect(user='root', password='Manish@1813',
        ...
        for i in total:
            ...
            if food1[0] in selected_foods:
                ...
                conn = pymysql.connect(user='root', password='Manish@1813',
                                       host='localhost', database='rest_review_db')
                qur = 'UPDATE reviews_table SET good_review=%d,bad_review=%d,customer=%d WHERE food="%s"' % (
                    p, n, c, food_name)
                mycur = conn.cursor()
                mycur.execute(qur)
                conn.commit()
...
lbl1 = Label(root1, text="Owner Username", bd=5, width=20,
             height=2, relief='ridge', font=('Arial', 12))
lbl2 = Label(root1, text="Password", bd=5, width=20,
             height=2, relief='ridge', font=('Arial', 12))
e1 = Entry(root1, bd=5, width=11, textvariable=a1,
           relief='ridge', font=('Arial', 20))
e2 = Entry(root1, bd=5, width=11, textvariable=a2,
           relief='ridge', font=('Arial', 20), show='*')
